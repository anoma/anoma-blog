...
Finding a good arithmetization can be cast as the problem of fitting your check into as thin a matrix as possible. We have a structure, $D$, that we want to check with a predicate, $P$. We want to create a function, $f$, that turns any given $D$ satisfying $P$ into a matrix, and devise a predicate over matricies, $P'$, such that $P'(m) \equiv (\exists d. f(d) = m)$. Ideally, the width of $f(d)$ should be $O(1)$ with respect to the size of $d$. Of course, we also want $P'$ to be equivalent to a low-degree polynomial equation, but I'm going to ignore that for now.

...
A similar system exists to eliminate the need for quantifiers in first-order logic. "Relation algebra" is a variable free alternative to first-order logic. This system is extremely interesting
...
Based on my understanding, it's using a conbinatorial construction which exploits and isomorphism between tree expressions and dyck wards to linearize tree expressions in a way that's more SNARK friendly. I think there is a lot of potential in using latice paths, pattern-avoiding permutations, and other linear combinatorial objects to create effective arithmetizations for higher-dimensitonal combinatorial structures. I don't want to give the impression that this post is about "the one true way", but I think it's better than alternatives I've seen, but there's so much low hanging fruit all over the design space that's worth exploring.
